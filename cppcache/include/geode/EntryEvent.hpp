...
class APACHE_GEODE_EXPORT EntryEvent {
 ...
 public:
  ...
  std::shared_ptr<CacheableKey> getKey() const { return m_key; }
   ...
   */
  std::shared_ptr<Cacheable> getOldValue() const { return m_oldValue; }

  /** If the event is a destroy or invalidate operation, then the new value
   * will be nullptr.
   * @return the updated value from this event
   */
  std::shared_ptr<Cacheable> getNewValue() const { return m_newValue; }
